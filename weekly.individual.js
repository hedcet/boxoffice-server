...
(async () => {
  const group = "";
  const name = /^narayan/i;
  const displayName = "NarayaneenteMoonnanmakkal";
  let image = ""; // bms/ptm image-url
  const start_date = moment("2025-02-07", ["YYYY-MM-DD"]);
  const end_date = moment("2025-02-21", ["YYYY-MM-DD"]);
